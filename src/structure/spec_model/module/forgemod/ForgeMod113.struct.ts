...
export class ForgeModStructure113 extends BaseForgeModStructure<ModsToml> {
    ...
    protected processZip(zip: StreamZip, name: string, path: string): ModsToml {
        ...
        if (raw) {
            ...
            this.logger.error(`ForgeMod ${name} does not contain mods.toml or neoforge.mods.toml file.`)
        }
        ...
        if (x != null && Array.isArray(x.mods)) {
        ...
        return this.modMetadata[name] as ModsToml
    }
