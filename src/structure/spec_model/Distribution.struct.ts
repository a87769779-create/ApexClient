...
export class DistributionStructure implements SpecModelStructure<Distribution> {
    ...
    public async getSpecModel(): Promise<Distribution> {
        ...
        console.log('DEBUG: distroMeta:', JSON.stringify(distroMeta, null, 2));
        if (!distroMeta) console.error('DEBUG: distroMeta is falsy!');
        if (!distroMeta.meta) console.error('DEBUG: distroMeta.meta is missing!');
