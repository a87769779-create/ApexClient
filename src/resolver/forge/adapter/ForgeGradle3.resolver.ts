...
export class ForgeGradle3Adapter extends ForgeResolver {
    ...
    private configure(): void {
        ...
        const isNeoForge = this.minecraftVersion.isGreaterThanOrEqualTo(new MinecraftVersion('1.21')) && this.forgeVersion.startsWith('21.')
        const forgeGroup = isNeoForge ? 'net.neoforged' : LibRepoStructure.FORGE_GROUP
        const forgeArtifact = isNeoForge ? 'neoforge' : LibRepoStructure.FORGE_ARTIFACT
        const forgeVersion = isNeoForge ? this.forgeVersion : this.artifactVersion
    ...
    private async processWithInstaller(installerPath: string): Promise<Module> {
        ...
        let installerOutputDir = this.repoStructure.getForgeCacheDirectory(this.artifactVersion)
        let doInstall = true

        if (await pathExists(installerOutputDir)) {
            if (this.invalidateCache) {
                ForgeGradle3Adapter.logger.info(`Removing existing cache ${installerOutputDir}..`)
                await remove(installerOutputDir)
            } else {
                doInstall = false
                ForgeGradle3Adapter.logger.info(`Using cached results at ${installerOutputDir}.`)
            }
        } else {
            const fallbackDir = this.repoStructure.getForgeCacheDirectory(this.forgeVersion)
            if (await pathExists(fallbackDir)) {
                if (this.invalidateCache) {
                    ForgeGradle3Adapter.logger.info(`Removing existing cache ${fallbackDir}..`)
                    await remove(fallbackDir)
                    await mkdirs(installerOutputDir)
                } else {
                    doInstall = false
                    installerOutputDir = fallbackDir
                    ForgeGradle3Adapter.logger.info(`Using cached results at ${installerOutputDir}.`)
                }
            } else {
                await mkdirs(installerOutputDir)
